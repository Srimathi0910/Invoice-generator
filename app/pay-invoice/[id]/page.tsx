        ...
        <div className="border border-gray-400 p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <p className="text-lg font-bold">
              Amount: Rs.{invoice.totals.grandTotal.toLocaleString()}
            </p>
          ...
          <div className="space-y-2 text-sm">
            ...
            >
              {paying
                ...
                : invoice.status === "Paid"
                  ? "Paid"
                  : `Pay Now Rs.${invoice.totals.grandTotal.toLocaleString()}`}
            </button>
          </div>
        </div>
...
const MenuItem = ({ icon, label, isActive, onClick }: any) => (
  <div
    onClick={onClick}
    className={`flex flex-row gap-2 items-center cursor-pointer whitespace-nowrap ${isActive ? "text-[#8F90DF] underline underline-offset-4 pb-1" : "text-black"}`}
  >
    {icon}
    <span>{label}</span>
  </div>
